...
`SPEC.md` is the V1 baseline; each major version's changes live in its own `SPEC V<N>.md` delta file. Current chain: `SPEC.md` → `SPEC V3.md` → `SPEC V4.md` → `SPEC V5.md` → `SPEC V6.md` → `SPEC V7.md` → `SPEC V8.md`.
...
static/
  ...
  js/player.js    # rejoin_token persisted to localStorage; connect handler prefers silent rejoin over name entry (SPEC V5.md)
...
- No reconnection identity matching *between devices*. Roster entries are always durable. On the *same* device, a buzz identity now persists across reconnects via a rejoin token (SPEC V5.md) — only a token-less device (first join, a different device, cleared storage) gets a fresh, disposable buzz identity.
...
Integration tests with the Flask-SocketIO test client: join → buzz → queue broadcast; room validation (valid/invalid/case-insensitive); late joiner behind frozen queue; `player:rejoin` restores identity + queue state, and survives a stale disconnect arriving after the new connection (SPEC V5.md); `host:player_remove` kicks the target socket with `player:removed` before disconnecting it, and is rejected once live (SPEC V6.md); `host:roster_remove` discards scores and broadcasts `state:scores` to every host tab, and is rejected before Start (SPEC V7.md). No browser/E2E tooling.
...
- **B1 (state machine):** `host:question_reveal` / `host:answer_reveal` / `host:question_cancel`, the queue lifecycle (SPEC V3.md §4), scoring panel relocated into the reveal flow. Server-side only — testable via the Flask-SocketIO test client, no presentation view yet. `question_submit` intentionally stays ungated in B1 (callable without a prior reveal, exactly like today) — hard-gating it is deferred to B2, built together with the `host.js` rewire that's the only thing that would ever trigger a reveal from the real UI. This is not an oversight; gating now would risk breaking the currently-deployed scoring flow if B1 ships before B2 is ready.
- **B2 (presentation view):** `/present/<join_code>/<host_token>` route, `state:presentation` protocol, template/JS. The visual payoff; depends on B1 already emitting correct state. Also includes the `host.js` control-center rewire (tile click → reveal panel, wired to `question_reveal`/`answer_reveal`/`question_cancel`) and the `question_submit` hard gate deferred from B1 — B2 is what actually drives the reveal state machine, not just what displays it, since nothing else in the sequencing ever will.
