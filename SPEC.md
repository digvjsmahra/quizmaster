...
- **Media filenames are matched by base name only, extension ignored on both sides.** A cell can reference `poster` or `poster.png` interchangeably; the actual file in `media/` can be named `poster` or `poster.png` (or even `poster.jpg`) — whatever's on either side, only the part before the first `.` is compared, case-sensitively. The real image format is never taken from a claimed extension — it's detected from the file's content (magic-byte sniffing: png / jpeg / gif / webp). This means a file named `poster.png` that's actually a JPEG still works fine; the app doesn't care what anyone called it, only what it actually is. If `media/` contains two files whose base names collide once extensions are stripped (e.g. `poster.png` and `poster.jpg` together), that's a validation error — the QM keeps one, no attempt is made to guess which was meant.

**Validation (fail loudly, at upload time, in the browser):**
- The whole file is validated in a single pass — every row's errors are collected and reported together, not just the first bad row. This holds even when a required column is entirely missing, or `media/` has a base-name collision: each is reported once as a bundle-level error, but row-level checks that don't depend on it (other media filenames, duplicate detection) still run, so a QM sees everything wrong in one upload rather than fixing issues one category at a time across repeated attempts.
- Structural errors reported per row with row number and reason: missing required field, non-numeric value, duplicate `question_id`, empty question+media pair, a media reference matching no file, or matching a file whose content isn't a recognized image format.
- Every filename in `question_media`/`answer_media` (split on comma, trimmed) must resolve to exactly one file in `media/` by base name → error if not found, or if the base name is ambiguous (collision).
- Files in `media/` referenced by no row → warning (not an error).
- Error messages avoid internal vocabulary (no raw `question_id`, no internal folder-path references) and are actionable, not just descriptive — e.g. a missing required column lists the columns actually found in the file so a semantic mismatch (`points` typed instead of `value`) is visible even when it can't be silently auto-corrected.
...
**Media storage/serving:** extracted media is held in a temp dir scoped to the room, wiped on restart, and served only via a host-secret route (`/media/<join_code>/<host_token>/<filename>`). The route serves the exact resolved filename from upload — extension-less on disk if that's how it arrived — and sets `Content-Type` from the same content-sniffed format, not the filename. Media URLs are emitted only to host-secret and presentation-secret sockets.
